...
% Meta Params
    L_Nx = 8; %10e6;         % number of rays to cast per trial
    L_Ny = 8;
    R_N = 8;
    debug = false;
    N_to_plot = 100000;
...
% 
% %         if trial == 1
% %             title_txt = [num2str(D_d(d)*1000) 'mm (' num2str(detector_hits(trial)) ' hits)'];
% %         else
% %             title_txt = [num2str(detector_hits(trial)) ' hits'];
% %         end
%         
% %         title(convertCharsToStrings(title_txt))
% %         x_txt=['v = ' num2str(v(trial)) 'm'];
% %     %     txt_subtitle = ['time = ' num2str(elapsed(trial)/60) ' min'];
% %        xlabel([convertCharsToStrings(x_txt)]);%, convertCharsToStrings(txt_subtitle )])
%         if (trial == 1) && (d == 1)
%             lgd = legend('source pt', 'detected source pt', 'PD hit' );
%             set(lgd,'location','east outside')
%         end
% %        axis equal
%     end
%     hold off
...
%     % plot(v,all);
% %     lgd=legend('calculated (point PD)', ['simulated (' num2str(D_d(d)*1000) 'mm PD)']);%, 'total hits (include redundant src pts)')
...
% end
%%
disp(['Simulation ended at: ' char(datetime)]);

% fid = fopen(fn, 'w');
% fprintf(fid, '%d  %d  %d  %d  %d  R_d=%.3f  D_d(d)=%.3f  h=%.2f  N=%.2e  T=%.2fs  Done on %s\n', detector_hits, R_d, D_d(d),h, N,elapsed, datetime('now'));
% fclose(fid);